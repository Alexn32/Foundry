...
class FounderOnboarding {
  ...
  async evaluateAnswerQuality(category, answer, conversationHistory) {
    const prompt = `Evaluate if this answer provides sufficient detail for a business profile.

Category: ${category}
Answer: "${answer}"

Check if the answer covers these aspects:
${this.getRequirementsForCategory(category)}

Respond with ONLY a JSON object:
{
  "sufficient": true/false,
  "missing": ["what's missing"],
  "follow_up_question": "specific question to ask to get missing info",
  "clarity_score": 1-10
}`;
  ...
  async extractInformation(category, answer, previousResponses) {
    ...
    const prompt = `Extract structured information from these answers about ${category}.

Answers:
${allResponses}

Extract and return ONLY a JSON object with relevant fields for ${category}.
Be specific and concrete. If information is missing, use null.`;
